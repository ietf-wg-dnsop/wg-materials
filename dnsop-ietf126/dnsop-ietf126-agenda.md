# DNS Operations (DNSOP) Working Group

## IETF 126

### Chairs

* Benno Overeinder <benno@nlnetlabs.nl>
* Ondrej Sury <ondrej+dnsop-chairs@sury.org>

### Secretaries

* Peter Thomassen <peter@desec.io>
* Shumon Huque <shuque@gmail.com>

### Technical Advisor

* Jim Reid <jim@rfc1035.com>

### Responsible Area Director

* Mohamed Boucadair <mohamed.boucadair@orange.com>

### Document Status

* [Datatracker](https://datatracker.ietf.org/wg/dnsop/documents/)

### Social Media

* [Bluesky @ietf-wg-dnsop.bsky.social](https://bsky.app/profile/ietf-wg-dnsop.bsky.social)
* [Mastodon @ietf_wg_dnsop@hachyderm.io](https://hachyderm.io/@ietf_wg_dnsop)


# Session I

* Date: Monday, July 20, 2026
* Time: 16:30-18:30 GMT+2
* Room: Grand Park Hall 2

## Agenda

### Administrativia

* Opening, Note Well and Chairs Updates, Chairs, 10 min

### Current Working Group Business

*   Integration of DNS Domain Names into Application Environments: Motivations and Considerations
    - https://datatracker.ietf.org/doc/draft-ietf-dnsop-integration/
    - Andrew Kaizer <akaizer@verisign.com>, 10 min
    - Chairs Action:

*   DNSSEC Key Restore
    - https://datatracker.ietf.org/doc/draft-ietf-dnsop-dnssec-keyrestore/
    - Florian Obser <florian+ietf@narrans.de>, 10 min
    - Chairs Action:

*   Automating DNS Delegation Management via DDNS
    - https://datatracker.ietf.org/doc/draft-ietf-dnsop-delegation-mgmt-via-ddns/
    - Johan Stenstam <johan.stenstam@internetstiftelsen.se>, 10 min
    - Chairs Action:

### For Consideration

*   Disclosure of Negative Trust Anchors in DNS Responses
    - https://datatracker.ietf.org/doc/draft-farrokhi-dnsop-ede-nta/
    - Joe Abley <jabley@strandkip.nl>, 15 min
    - Chairs Action:

*   Signalling a Zone Cut to Nowhere in the DNS
    - https://datatracker.ietf.org/doc/draft-jabley-dnsop-zone-cut-to-nowhere/
    - Joe Abley <jabley@strandkip.nl>, 15 min
    - Chairs Action:

*   Add TTLs to DNS errors
    - https://datatracker.ietf.org/doc/draft-homburg-dnsop-dettl/
    - Philip Homburg <pch-dnsop-7@u-1.phicoh.com>, 10 min
    - Chairs Action:

### DNS DISPATCH

*   Domain Key Authorities (DKA): DNS-Designated Public Key Distribution for Email-Address Identifiers
    - https://datatracker.ietf.org/doc/draft-swaminathan-dka-framework/
    - Kishore Swaminathan <k.s.swaminathan@live.com>, 5 min

*   Domain Authority (DA): A DNS-Designated Service Endpoint for Domain Information
    - https://datatracker.ietf.org/doc/draft-swaminathan-da/
    - Kishore Swaminathan <k.s.swaminathan@live.com>, 5 min

*   DNS Extensions to Energy Efficiency as a Service(EEAS)
    - https://datatracker.ietf.org/doc/draft-zhu-dnsop-de-eeas/
    - Huahong Zhu <zhuhh11@chinatelecom.cn>, 10 min

*   UTXO Domain Name System (UTXO6-DNS): Base Protocol and PRN Regulatory Extensions
    - https://datatracker.ietf.org/doc/draft-guorong-utxo-dns/
    - Guorong Tian <foundation@utxocp.cn>, 10 min

#### Time Permitting

*   DNS-Based Address Mapping Record (AMR) for IPv4/IPv6 Mapping in IPv6-only Networks
    - https://datatracker.ietf.org/doc/draft-dong-dnsop-dns-record-amr/
    - Guozhen Dong <donggz@chinatelecom.cn>


## Session II

* Date: Friday, July 24, 2026
* Time: 11:30-12:30 GMT+2
* Room: Grand Park Hall 2

### Administrativia

* Opening, Note Well and Chairs Updates, Chairs, 5 min

### Current Working Group Business

*   Delegation Revalidation by DNS Resolvers
    - https://datatracker.ietf.org/doc/draft-ietf-dnsop-ns-revalidation/
    - Shumon Huque <shuque@gmail.com>, 5 min
    - Chairs Action:

### For Consideration

*   Parent-Centric Delegation Handling in DNS Resolvers
    - https://datatracker.ietf.org/doc/draft-sury-dnsop-parent-centric-resolver/
    - Ondřej Surý <ondrej@isc.org>, 10 min
    - Chairs Action:

*   DNS Catalog Zone Properties for Zone Transfers
    - https://datatracker.ietf.org/doc/draft-axu-dnsop-catalog-zone-xfr-properties/
    - Willem Toorop <willem@nlnetlabs.nl>, 15 min
    - Chairs Action:

*   Optimistic DNS
    - https://datatracker.ietf.org/doc/draft-gakiwate-dnsop-optimistic-dns/
    - Gautam Akiwate <gakiwate@apple.com>, 10 min
    - Chairs Action:

*   DNS Root Server System Usage Considerations
    - https://datatracker.ietf.org/doc/draft-hardaker-dnsop-rss-usage-considerations/
    - Wes Hardaker <wjhns1@hardakers.net>, 15 min
    - Chairs Action:
